...
PUBLIC int readchar P0()
{
    ...
    ch = ((unsigned char *) ibuf)[0];  ++ibuf;  /* It works with any C compiler. */
#else
...
{
    flushout();
    if ((INT32T) offset < 0 || lseek(outfd, cast_lseek_offset(offset), SEEK_SET) != cast_lseek_offset((INT32T) offset))
