...
## Differences between the C compilers ACK and BCC

These are compared below:

* ACK: The ACK 3.1 toolchain (C compiler driver, C compiler frontend, C
  compiler backend, assembler, linker, C preprocessor) that ships with Minix
  1.5.10 i86 as executable program files (binary only, no source).
* BCC v0: The C compiler driver, C compiler frontend--backend (with the C
  preprocessor integrated), assembler and linker by Bruce Evans,
  targeting Minix 1.5.10 i86 and i386 earlier, on 1990-06-10 (v0), running on
  Minix i86 host
  ([bccbin16.tar.Z](https://github.com/pts/pts-minix-1.5.10-hdd-image/releases/download/minix-1.5.10-i386-patches/bccbin16.tar.Z))
  and on Minix i386 host
  ([bccbin32.tar.Z](https://github.com/pts/pts-minix-1.5.10-hdd-image/releases/download/minix-1.5.10-i386-patches/bccbin32.tar.Z)).
  This is also a binary-only release.
* BCC v3: C compiler frontend--backend (with the C preprocessor integrated)
  by Bruce Evans, released on 1993-07-1, accompanied by a compatible C
  compiler driver, assembler and linker (all by Bruce Evans, released in
  1990--1993). This is the C compiler minixbcc is based on.
* BCC: Both BCC v0 and BCC v3.

The differences are:

* For ACK, in .c and .s files, only the first 8 characters of the
  identifiers (variable and function names) are significant. BCC is mostly
  unlimited. The 8 characters are actually 7, because global symbols start
  with an underscore (`_`).
* For ACK, `static' is ignored: all global variables and functions are
  exported, and may conflict with each other.
* ACK can generate optimized machine code (with `cc -O`), BCC (`bcc -O`)
  ignores the `-O` flag, and always generates uninitialized code.
* ACK supports functions returning a struct, BCC doesn't.
* ACK doesn't use object files (*.o), the ACK C compiler generates
  dictionary-packed assembly source files (*.s), which can be unpacked
  using the libupack tool.
* For ACK, the assembler and linker are combined to the *asld* tool.
* The ACK assembly language is similar but different from the BCC assembly
  language.
* ACK comes with and uses a C preprocessor (`/usr/lib/cpp`), BCC has the
  preprocessor integrated to the C compiler backend (`/local/bin/sc`), and
  BCC can't generate a preprocessed output file.
* The ACK *asld* assembler--linker often crashes with out-of-memory for
  more complex projects.
* The ACK C compiler displays some useless warnings.
* For ACK, merged I&D is the default, specify `cc -i` or `asld -i` to
  enable separate I&D. For BCC, it depends on the version: BCC v0 `bcc`
  and `bcc -i` both run `ld -i` by default.
* For ACK, `cc -s` means to add symbols (symbols stripped by default), for
  BCC (and GCC), `cc -s` means to strip symbols (symbols added by default).
* ACK has this bug (BCC doesn't): `(int) (unsigned char) (int) (x)` is
  incorrectly simplifted to `(int) (x)` or `(int) (signed char) (int) (x)`,
  and not truncated to 8 bits.
* ACK has this bug (BCC doesn't): in optimizing mode (`cc -O`), compiling
  `a->f1 = a->f2 = a->f3 = 5;` crashes.
* ACK has this bug (BCC doesn't): in optimizing mode (`cc -O`), compiling
  `symptr->value = comsz[seg = flags & SEGM_MASK];` crashes. (What is the
  pattern here?)
* BCC supports the i86 (8086) and i386 target architectures selectable at
  runtime, ACK supports only i86 (8086).
* ACK doesn't support initializers for unions, BCC supports them correctly
  (i.e. it initializes the first member).
* The ACK C compiler backend generates dictionary-packed assembly source
  files (*.s), which can be unpacked using the libupack tool. BCC C compiler
  backend generates ASCII (unpacked) assembly source files.
* The ACK assembler accepts both dictionary-packed and ASCII (unpacked)
  assembly source files (*.s), the BCC assembler accepts only the latter
  (unpacked).
* The ASK C compiler driver instructs *asld* to add `/usr/lib/ends.s` as the
  last object file. There is no such file for BCC, because the BCC linker
  adds the symbols defined there for ACK.
* The ACK autogenerated symbol *endbss* is called *_end* for BCC.
* The ACK autogenerated symbol *enddata* is called *_edata* for BCC.
* The ACK autogenerated symbol *etext* is called *_etext* for BCC.
* The ACK assembler doesn't support hexadecimal literals, BCC supports them
  with the `$` prefix.
* The ACK assembler requires byte-sized instructions to have the `b` suffix,
  e.g. it accepts `movb al, *1`, but it fails for `mov al, *1`. BCC accepts
  both forms.
* The assembly (assembler input) syntax is different.
* BCC generates incorrect code for `-(unsigned) (char*) (p) & 7` (it ands to
  *al*, but the entire *ax* would be correct), ACK is correct.
* BCC v0 is unnecessarily stricter than BCC v0 and ACK about redefining a
  macro to a different value, for example BCC v0 fails for `#define P(x) ()`
  followed by the same, but with tab instead of space after the `P(x)`, and
  the others allow it.
* Both BCC and ACK support the floating-point types *float* and *double*,
  but they call different helper functions, e.g. for adding two *float*
  values, ACK calls *.cff* and *.adf8*, BCC v0 calls *fadd* (conflicting with
  the assembly instruction fadd*), BCC v3 calls *Fadd*.
* BCC doesn't supply the floating-point helper functions, they are available
  in the separate bccfp package, only for BCC v3 targeting i386.
* When building a library (.a archive file) with ACK, the order of the
  assembly--object files (.s) within the library is important: if a.o
  depends on b.o, then a.o should come first, otherwise the *asld* linker
  would ignore it. There is no such order restriction with BCC.
* When adding an assembly--object file (.s) to a library (.a archive file)
  with ACK, the first few assembly pseudo-ops must be `.define` (compressed
  byte 0xc8), not `.global`, otherwise the *asld* linker would ignore it.
  There is no such restriction with BCC.
