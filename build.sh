...
for a03 in 0 3; do
  if test "$a03" = 0; then  # i86.
    ...
    STRCB=''  # No additional string routines, C source files (*.c).
    STRC2B=''  # No additional string routines, C source files (*.c).
  else  # i386.
  ...
  # ($bccsupportdir) $SB $IB $LBB $LLB $MB
  for b in $STRPASMB $SB $IB $JB $LBB $LLB $MB; do
