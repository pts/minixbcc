...
for a03 in 0 3; do
  if test "$a03" = 0; then  # i86.
    LLB='laddl landl lcmpl lcoml ldecl ldivl ldivul leorl lincl lmodl lmodul lmull lnegl lorl lsll lsrl lsrul lsubl ltstl ldivmod'  # BCC compiler support for long arithmetic on little-endian (x86) longs, assembly source files (*.s) by Bruce Evans.
    STRASMB='memchr strcat strncat strchr strcoll strrchr strspn strstr strtok strxfrm strpbrk strcspn'  # Minix library string routines, assembly source files (*.s) created manually from to-be-preprecessed assembly source files (*.x) from Minix 1.5.10 /usr/src/lib/string . Patched by Bruce Evans.
    STRCCCB=''  # No additional string routines, C source files (*.c). These are implemented in STRASMB instead.
  else  # i386.
    LLB=''  # No additional BCC compiler support for long arithmetic on little-endian (x86) longs, assembly source files (*.s).
    STRASMB=''  # No additional Minix library string routines. These are implemented in STRCCCB instead.
    STRCCCB='memchr strcat strncat strchr strcoll strrchr strspn strstr strtok strxfrm strpbrk strcspn'  # Minix library string routines, C source files (*.c) without assembly from Minix 1.5.10 /usr/src/lib/string . Patched by Bruce Evans.
  fi
  ...
  for b in $STRCCCB $ANSIB $IBMB $OTHERB $POSIXB; do
    # bcc -v -"$a03" -c -O -D_MINIX -D_POSIX_SOURCE -o "$a03"/"$b".o "$b".c
    "$sc" -"$a03" -Iinclude -D_MINIX -D_POSIX_SOURCE -o "$a03"/"$b".s libc/"$b".c || exit "$?"
